...
export const InvestmentCSVImport = ({ onImport }: InvestmentCSVImportProps) => {
  ...
  const parseRows = (sourceRows: Record<string, unknown>[]) => {
    ...
    sourceRows.forEach((values, index) => {
      ...
      const quantity = parseFloat(String(values.quantity || values.amount || values.mnozstvi || values.množství || '0').replace(',', '.'));
    ...
    <div className="space-y-4">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
        <Alert>
          ...
          <AlertDescription>
            Můžeš importovat CSV i XLSX. Doporučená cesta je nejdřív zkusit export brokera. Pokud ho broker nemá
            nebo potřebuješ data upravit, stáhni si šablonu FIGR, vyplň ji a nahraj zpět.
          </AlertDescription>
        </Alert>
      ...
      {rows.length === 0 && (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 p-6 text-center text-sm text-muted-foreground">
          ...
          Stáhni si šablonu nebo export z brokera, zkontroluj první nákupy, prodeje a dividendy a potom soubor nahraj sem.
        </div>
      )}
    </div>
  );

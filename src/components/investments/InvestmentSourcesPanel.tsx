...
import { Building2, CalendarClock, Database, Plus, RefreshCw } from 'lucide-react';
...
const SourceAccountForm = ({ onSave }: { onSave: InvestmentSourcesPanelProps['onAddSourceAccount'] }) => {
  ...
  const applyProviderDefaults = (nextProvider: InvestmentProvider) => {
    ...
    if (nextProvider === 'investown' || nextProvider === 'fingood') {
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Jedno místo pro brokery, Investown, Edward i další platformy.</p>
          </div>
      ...
      <CardContent>
        {sourceAccounts.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/70 p-6 text-center text-sm text-muted-foreground">
            <Building2 className="mx-auto mb-3 h-6 w-6" />Začni přidáním Investownu, Edwarda nebo brokerského účtu.
          </div>
        ) : (

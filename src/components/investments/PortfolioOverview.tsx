...
export const PortfolioOverview = ({ portfolioSummary, loading }: PortfolioOverviewProps) => {
  ...
  const quickStats = [
    { label: 'Tržní portfolio', value: marketCurrentValue !== null ? formatCurrency(marketCurrentValue, reportingCurrency) : 'Chybí ceny', hint: 'Akcie, ETF, krypto' },
          ...
          <CardContent className={isMobile ? 'p-4' : 'p-6'}>
            <div className={`flex ${isMobile ? 'flex-col gap-4' : 'flex-col gap-5 lg:flex-row lg:items-end lg:justify-between'}`}>
              <div className="space-y-3">
                ...
                <div><p className="text-sm text-muted-foreground">Aktuální hodnota</p><p className={`${isMobile ? 'text-2xl' : 'text-3xl sm:text-4xl'} font-bold tracking-tight`}>{currentValue !== null ? formatCurrency(currentValue, reportingCurrency) : 'Chybí ocenění'}</p></div>

        ...
        <div className="space-y-6">
          {yearlyData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
              ...
              <p className="text-lg">Zatim nejsou k dispozici data pro report.</p>
            </div>
          ) : (
            <>
                ...
                {yearlyData.slice(-4).reverse().map((item) => (
                  <Card key={item.year}>
                    ...
                    <CardContent className="space-y-1">
                      <p className="text-sm">Prijmy: <span className="font-semibold">{formatCurrency(item.income)}</span></p>
                      <p className="text-sm">Vydaje: <span className="font-semibold">{formatCurrency(item.expenses)}</span></p>
                      <p className="text-sm">Investovano: <span className="font-semibold">{formatCurrency(item.investments)}</span></p>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Rocni souhrn</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-[320px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={yearlyData}>
                        ...
                        <Bar dataKey="income" fill="hsl(var(--primary))" name="Prijmy" />
                        <Bar dataKey="expenses" fill="hsl(var(--destructive))" name="Vydaje" />
                        <Bar dataKey="investments" fill="hsl(var(--success))" name="Investovano" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Vyvoj majetku podle mesicu</CardTitle>
                </CardHeader>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Vyvoj majetku ze snapshotu</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-[320px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={netWorthHistory}>
                        ...
                        <YAxis />
                        <Tooltip formatter={(value: number) => formatCurrency(value)} />
                        <Line type="monotone" dataKey="totalNetWorth" stroke="hsl(var(--primary))" strokeWidth={2} name="Cisty majetek" />
                        <Line type="monotone" dataKey="bankAssets" stroke="hsl(var(--warning))" strokeWidth={2} name="Banky" />
                        <Line type="monotone" dataKey="brokerAssets" stroke="hsl(var(--success))" strokeWidth={2} name="Brokerske ucty" />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
